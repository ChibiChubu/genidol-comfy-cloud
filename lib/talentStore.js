...
async function updateTalentAsset(id, key, value) {
  return withWriteLock(async () => {
    const talents = await readAll();
    const talent = talents.find((item) => item.id === id);
    if (!talent) return null;
    talent.assets = { ...(talent.assets || {}), [key]: value };
    await writeAll(talents);
    return talent;
  });
}

export { listTalents, getTalent, addTalent, deleteTalent, updateTalentOwner, updateTalentAsset, DATA_DIR };
